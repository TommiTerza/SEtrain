...
class TRA(nn.Module):
    ...
    def __init__(self, channels, gru_factory: Optional[Callable[..., nn.Module]] = None):
        super().__init__()
        if gru_factory is None:
            gru_factory = _default_gru_factory
        self.att_gru = gru_factory(channels, channels*2, num_layers=1, batch_first=True)
...
class GTConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, stride, padding, dilation,
                 use_deconv=False, gru_factory: Optional[Callable[..., nn.Module]] = None):
        super().__init__()
        self.use_deconv = use_deconv
        ...
        self.tra = TRA(in_channels//2, gru_factory=gru_factory)
...
class GRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, batch_first=True, bidirectional=False,
                 log_gru_inputs=False, log_file=None, gru_factory: Optional[Callable[..., nn.Module]] = None):
        super().__init__()
        ...
        if gru_factory is None:
            gru_factory = _default_gru_factory
        self.rnn1 = gru_factory(input_size//2, hidden_size//2, num_layers=num_layers,
                                batch_first=batch_first, bidirectional=bidirectional)
        self.rnn2 = gru_factory(input_size//2, hidden_size//2, num_layers=num_layers,
                                batch_first=batch_first, bidirectional=bidirectional)
...
class DeltaGRU(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: Optional[torch.Tensor] = None):
        ...
        if h0 is None:
            h_prev = self._init_hidden(batch_size, x)
        else:
            h_prev = h0
        h_prev_prev: Optional[torch.Tensor] = None
        prev_x_actual: Optional[torch.Tensor] = None

        outputs = []
        for t in range(seq_len):
            x_curr_actual = time_major[t]  # (B,input_size)
            x_t = x_curr_actual
            if prev_x_actual is not None and self._threshold_active(self.threshold_x):
                delta_x = (x_curr_actual - prev_x_actual).abs()
                reuse_prev = delta_x < self.threshold_x
                x_t = torch.where(reuse_prev, prev_x_actual, x_curr_actual)

            h_in = h_prev
            if h_prev_prev is not None and self._threshold_active(self.threshold_h):
                delta_h = (h_prev - h_prev_prev).abs()
                reuse_prev_h = delta_h < self.threshold_h
                h_in = torch.where(reuse_prev_h, h_prev_prev, h_prev)

            step_input = x_t.unsqueeze(0)  # (1,B,input_size) in time-major format
            step_out, h_curr = self.gru(step_input, h_in)
        ...
        if not outputs:
            empty = x.new_zeros(0, batch_size, self.num_directions * self.hidden_size)
            output = empty.transpose(0, 1) if self.batch_first else empty
            return output, h_prev

        output_time_major = torch.cat(outputs, dim=0)
        output = output_time_major.transpose(0, 1) if self.batch_first else output_time_major
        return output, h_prev
...
class DPGRNN(nn.Module):
    ...
    def __init__(self, input_size, width, hidden_size, log_gru_inputs=False, log_file_base=None,
        ...
        self.intra_rnn = GRNN(input_size=input_size, hidden_size=hidden_size//2, bidirectional=True,
                              log_gru_inputs=log_gru_inputs, log_file=intra_log, gru_factory=gru_factory)
        self.intra_fc = nn.Linear(hidden_size, hidden_size)
        self.intra_ln = nn.LayerNorm((width, hidden_size), eps=1e-8)

        self.inter_rnn = GRNN(input_size=input_size, hidden_size=hidden_size, bidirectional=False,
                              log_gru_inputs=log_gru_inputs, log_file=inter_log, gru_factory=gru_factory)
...
class Encoder(nn.Module):
    def __init__(self, tra_gru_factory: Optional[Callable[..., nn.Module]] = None):
        super().__init__()
        self.en_convs = nn.ModuleList([
            ...
            GTConvBlock(16, 16, (3,3), stride=(1,1), padding=(0,1), dilation=(1,1), use_deconv=False, gru_factory=tra_gru_factory),
            GTConvBlock(16, 16, (3,3), stride=(1,1), padding=(0,1), dilation=(2,1), use_deconv=False, gru_factory=tra_gru_factory),
            GTConvBlock(16, 16, (3,3), stride=(1,1), padding=(0,1), dilation=(5,1), use_deconv=False, gru_factory=tra_gru_factory)
        ])
...
class Decoder(nn.Module):
    def __init__(self, tra_gru_factory: Optional[Callable[..., nn.Module]] = None):
        ...
        self.de_convs = nn.ModuleList([
            GTConvBlock(16, 16, (3,3), stride=(1,1), padding=(2*5,1), dilation=(5,1), use_deconv=True, gru_factory=tra_gru_factory),
            GTConvBlock(16, 16, (3,3), stride=(1,1), padding=(2*2,1), dilation=(2,1), use_deconv=True, gru_factory=tra_gru_factory),
            GTConvBlock(16, 16, (3,3), stride=(1,1), padding=(2*1,1), dilation=(1,1), use_deconv=True, gru_factory=tra_gru_factory),
...
class GTCRN(nn.Module):
    ...
    def __init__(
        ...
        gru_factory = self._build_gru_factory(use_delta_gru, delta_gru_threshold_x, delta_gru_threshold_h)

        self.encoder = Encoder(tra_gru_factory=gru_factory)
        ...
        self.dpgrnn1 = DPGRNN(16, encoder_width, 16, log_gru_inputs=log_gru_inputs,
                              log_file_base=dp1_base, gru_factory=gru_factory)
        self.dpgrnn2 = DPGRNN(16, encoder_width, 16, log_gru_inputs=log_gru_inputs,
                              log_file_base=dp2_base, gru_factory=gru_factory)
        self.decoder = Decoder(tra_gru_factory=gru_factory)
    ...
    @staticmethod
    def _build_gru_factory(use_delta_gru: bool,
                           threshold_x: Optional[float],
                           threshold_h: Optional[float]) -> Callable[..., nn.Module]:
