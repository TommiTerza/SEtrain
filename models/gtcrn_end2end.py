...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        x = self.quant(x)
        x = self.dequant(self.bn(self.conv(x)))
        return self.act(x)
...
class GTConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, stride, padding, dilation, use_deconv=False):
        ...
        if not use_deconv:
            self.point_quant1 = torch.quantization.QuantStub()
            self.point_dequant1 = torch.quantization.DeQuantStub()
        else:
            self.point_quant1 = nn.Identity()
            self.point_dequant1 = nn.Identity()
        ...
        if not use_deconv:
            self.point_quant2 = torch.quantization.QuantStub()
            self.point_dequant2 = torch.quantization.DeQuantStub()
        else:
            self.point_quant2 = nn.Identity()
            self.point_dequant2 = nn.Identity()

        self.point_conv2 = conv_module(hidden_channels, in_channels//2, 1)
        self.point_bn2 = nn.BatchNorm2d(in_channels//2)
        if not use_deconv:
            self.point_quant3 = torch.quantization.QuantStub()
            self.point_dequant3 = torch.quantization.DeQuantStub()
        else:
            self.point_quant3 = nn.Identity()
            self.point_dequant3 = nn.Identity()
    ...
    def fuse_model(self):
        ...
        if fuse_groups:
            torch.quantization.fuse_modules(self, fuse_groups, inplace=True)
    ...
    def forward(self, x):
        ...
        h1 = self.point_dequant1(self.point_bn1(self.point_conv1(h1)))
        h1 = self.point_act(h1)
        h1 = nn.functional.pad(h1, [0, 0, self.pad_size, 0])
        h1 = self.point_quant2(h1)
        h1 = self.point_dequant2(self.depth_bn(self.depth_conv(h1)))
        h1 = self.depth_act(h1)
        h1 = self.point_quant3(h1)
        h1 = self.point_dequant3(self.point_bn2(self.point_conv2(h1)))
...
class GTCRN(nn.Module):
    ...
    def prepare_qat(self, backend='fbgemm'):
        ...
        for module in self.modules():
            ...
            if isinstance(module, nn.modules.conv._ConvNd) and not isinstance(module, nn.ConvTranspose2d):
                module.qconfig = qat_qconfig
                continue

            if 'ConvBn' in module.__class__.__name__:
                module.qconfig = qat_qconfig
    ...
    def convert_qat(self, inplace=False):
