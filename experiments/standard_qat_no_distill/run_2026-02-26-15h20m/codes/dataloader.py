...
def _resolve_default_data_root() -> Path:
    env_root = os.environ.get("SETRAIN_DATA_ROOT")
    if env_root:
        return Path(env_root).expanduser().resolve()

    # Prefer the shared dataset managed by the sibling repo: NNs/SEtrain
    sibling_root = Path(__file__).resolve().parents[2] / "SEtrain" / "data" / "voicebank-demand-16k"
    if sibling_root.exists():
        return sibling_root

    return Path(__file__).resolve().parent / "data" / "voicebank-demand-16k"
...
def prepare_voicebank_dataset(root: Path) -> None:
    ...
    try:
        ...
        msg = (
            "HuggingFace `datasets` is not installed, so the dataloader cannot auto-download VoiceBank-DEMAND.\n"
            "Options:\n"
            "  1) Install it: `pip install datasets` (and optionally `pip install soundfile`).\n"
            "  2) Point `dataset_root` (or $SETRAIN_DATA_ROOT) to an existing local copy.\n"
    ...
    try:
        ...
        msg = (
            "Failed to load the VoiceBank-DEMAND dataset. This is often caused by optional audio-decoding\n"
            "dependencies (e.g., FFmpeg / torchcodec) not being available or compatible.\n"
            "Workarounds:\n"
            "  * Download/prepare the WAVs manually and set `dataset_root` (or $SETRAIN_DATA_ROOT).\n"
            "  * Install FFmpeg system libraries so optional audio decoders work.\n"
...
class VoiceBankDemandDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if dataset_root is None:
            self.dataset_root = _resolve_default_data_root()
        else:
    ...
    def _crop_or_pad_pair(self, noisy: np.ndarray, clean: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Keep noisy/clean perfectly aligned by applying identical crop/pad offsets.
        """
        pair_len = min(noisy.shape[0], clean.shape[0])
        noisy = noisy[:pair_len]
        clean = clean[:pair_len]

        if pair_len >= self.L:
            if self.train and self.random_start_point and pair_len > self.L:
                start = random.randint(0, pair_len - self.L)
            elif self.train:
                start = 0
            else:
                start = (pair_len - self.L) // 2
            end = start + self.L
            return noisy[start:end], clean[start:end]

        pad_needed = self.L - pair_len
        if self.train and self.random_start_point:
            pad_left = random.randint(0, pad_needed)
        else:
            pad_left = pad_needed // 2
        pad_right = pad_needed - pad_left
        noisy = np.pad(noisy, (pad_left, pad_right), mode="constant")
        clean = np.pad(clean, (pad_left, pad_right), mode="constant")
        return noisy, clean
    ...
    def __getitem__(self, idx: int):
        ...
        noisy, clean = self._crop_or_pad_pair(
