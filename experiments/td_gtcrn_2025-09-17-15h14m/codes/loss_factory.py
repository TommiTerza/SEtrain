...
def si_snr(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    ratio = torch.sum(proj * proj, dim=-1, keepdim=True) / (torch.sum(noise * noise, dim=-1, keepdim=True) + eps)
...
class WaveformLoss(nn.Module):
    """Composite loss: SI-SNR plus multi-resolution STFT terms."""
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        sisnr = -si_snr(pred, target).mean()
        mag_loss, complex_loss = self.mrstft(pred, target)
        return self.si_snr_weight * sisnr + self.mag_weight * mag_loss + self.complex_weight * complex_loss


__all__ = ["WaveformLoss", "si_snr", "MultiResolutionSTFTLoss", "STFTLoss", "STFTConfig"]
