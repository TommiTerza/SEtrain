...
class GTCRN(nn.Module):
    ...
    def prepare_qat(self, backend='fbgemm', quantize_deconv=False):
        ...
        deconv_qconfig = None
        if quantize_deconv:
            # Quantized ConvTranspose2d doesn't support per-channel weight quantization (e.g. default fbgemm).
            # Use per-tensor weight fake-quant for deconvs to enable conversion.
            per_tensor_weight_qconfig = quant.get_default_qat_qconfig('qnnpack')
            deconv_qconfig = quant.QConfig(activation=qat_qconfig.activation, weight=per_tensor_weight_qconfig.weight)
        ...
        for module in self.modules():
            ...
            if isinstance(module, GTConvBlock):
                if module.use_deconv:
                    if not quantize_deconv:
                        continue
                    conv_qconfig = deconv_qconfig
                else:
                    conv_qconfig = qat_qconfig
                ...
                module.point_conv1.qconfig = conv_qconfig
                module.depth_conv.qconfig = conv_qconfig
                module.point_conv2.qconfig = conv_qconfig
