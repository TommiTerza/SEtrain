...
- Each block learns to reproduce exactly the tensor that the teacher block outputs.
...
class SingleBlockIOCapture:
    ...
    def __init__(self, model: torch.nn.Module, block_name: str):
        self.block_name = block_name
        self.block_input: torch.Tensor | None = None
        self.block_output: torch.Tensor | None = None

        named_modules = dict(model.named_modules())
        if block_name not in named_modules:
            raise RuntimeError(f"Block '{block_name}' not found in teacher model.")
        target = named_modules[block_name]

        self.pre_handle = target.register_forward_pre_hook(self._pre_hook)
        self.post_handle = target.register_forward_hook(self._post_hook)

    def _pre_hook(self, _module, args):
        if not args:
            raise RuntimeError(f"Teacher block '{self.block_name}' got no positional input.")
        x = args[0]
        if isinstance(x, (tuple, list)):
            if not x:
                raise RuntimeError(f"Teacher block '{self.block_name}' got empty tuple/list input.")
            x = x[0]
        if not torch.is_tensor(x):
            raise RuntimeError(f"Teacher block '{self.block_name}' input is not a tensor.")
        self.block_input = x.detach()

    def _post_hook(self, _module, _args, output):
        if isinstance(output, (tuple, list)):
            if not output:
                raise RuntimeError(f"Teacher block '{self.block_name}' returned empty tuple/list output.")
            output = output[0]
        if not torch.is_tensor(output):
            raise RuntimeError(f"Teacher block '{self.block_name}' output is not a tensor.")
...
class IndependentBlockwiseDistiller:
    def __init__(
        ...
        # Default behavior: train conv/deconv paths only; keep TRA fixed.
        self.train_tra = bool(self.block_cfg.get("train_tra", False))
        ...
        # Whether to copy GRUs only after independent block training + assembly.
        self.copy_gru_after_training = bool(
            self.block_cfg.get(
                "copy_gru_from_teacher_after_training",
                self.block_cfg.get("copy_gru_from_teacher", False),
            )
        )
        ...
        for name in self.block_names:
            print(f"  - {name}")
        if not self.train_tra:
            print("TRA submodules in GTConvBlock are frozen (copied from teacher).")
    ...
    def _build_block_instance(self, block_name: str) -> torch.nn.Module:
        template_modules = dict(self.student_template.named_modules())
        teacher_modules = dict(self.teacher_model.named_modules())
        block = deepcopy(template_modules[block_name]).to(self.device)
        block.load_state_dict(self.initial_block_states[block_name], strict=True)
        block.train()

        if isinstance(block, GTConvBlock) and not self.train_tra:
            # Keep TRA fixed to teacher so conv/deconv parameters are the only trainable part.
            block.tra.load_state_dict(teacher_modules[block_name].tra.state_dict(), strict=True)
            for parameter in block.tra.parameters():
                parameter.requires_grad_(False)
            # Keep GRU in training mode to allow autograd through cuDNN RNN ops.
            # Parameters remain frozen via requires_grad=False.
            block.tra.train()
    ...
    def _train_one_block(self, block_name: str) -> tuple[dict[str, Any], dict[str, Any]]:
        block = self._build_block_instance(block_name)
        ...
        capture = SingleBlockIOCapture(self.teacher_model, block_name)
        ...
        for epoch in range(1, self.epochs_per_block + 1):
            self._apply_qat_schedule_to_block(block, epoch)
            ...
            for step, (noisy, _clean) in enumerate(progress, 1):
                ...
                teacher_input = capture.block_input
                teacher_output = capture.block_output
                if teacher_input is None or teacher_output is None:
                    raise RuntimeError(f"Missing teacher IO capture for block '{block_name}'.")

                optimizer.zero_grad(set_to_none=True)
                pred = _forward_block_standalone(block, teacher_input, teacher_output)
                loss = self._compute_loss(pred, teacher_output)
                loss.backward()

                if self.clip_grad_norm_value > 0:
                    torch.nn.utils.clip_grad_norm_(block.parameters(), self.clip_grad_norm_value)
            ...
            if epoch_loss < best_loss:
                ...
                best_epoch = epoch
                best_state = deepcopy(block.state_dict())
    ...
    def _assemble_final_model(
        ...
        final_model = deepcopy(self.student_template).to(self.device)
        final_modules = dict(final_model.named_modules())

        for name, payload in trained_block_states.items():
            final_modules[name].load_state_dict(payload["model"], strict=True)

        if self.copy_gru_after_training:
            # Do not overwrite GRUs that belong to independently trained blocks
            # (e.g., TRA.att_gru inside GTConvBlock).
            skip_prefixes = tuple(f"{block_name}." for block_name in self.block_names)
            copied_modules, copied_params = _copy_gru_from_teacher(
                student_model=final_model,
                teacher_model=self.teacher_model,
                freeze=False,
                skip_prefixes=skip_prefixes,
            )
            print(
                f"Copied GRUs from teacher into assembled model: "
                f"{copied_modules} modules, {copied_params} params."
            )
        ...
        final_checkpoint = {
            ...
            "copy_gru_from_teacher_after_training": self.copy_gru_after_training,
        }
    ...
    def train(self) -> None:
        ...
        try:
            ...
            for i, block_name in enumerate(self.block_names, 1):
                ...
                print(
                    f"Finished {block_name}: best_loss={summary['best_loss']:.6f} "
