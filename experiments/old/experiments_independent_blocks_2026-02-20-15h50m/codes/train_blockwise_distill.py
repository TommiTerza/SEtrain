    ...
    def _train_one_block(self, block_name: str) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        for epoch in range(1, self.epochs_per_block + 1):
            ...
            for step, (noisy, _clean) in enumerate(progress, 1):
                ...
                pred = _forward_block_standalone(training_unit, teacher_input, teacher_output)
    ...
    def _assemble_final_model(
        ...
        for name, payload in trained_block_states.items():
            ...
            if payload_type == "gtconv_conv_path":
                missing_keys, unexpected_keys = final_modules[name].load_state_dict(
                    payload["model"],
                    strict=False,
                )
                ...
                if bad_missing or unexpected_keys:
                    raise RuntimeError(
                        f"Failed partial load for GTConv block '{name}'. "
        ...
        if self.copy_gru_after_training:
            # Never copy TRA GRUs inside GTConv blocks.
            skip_prefixes = tuple(
                f"{module_name}.tra."
                for module_name, module in final_model.named_modules()
