...
def _apply_grouped_conv2d_transpose(
    ...
    if (filters % groups) != 0:
        raise ValueError(f"{name}: filters={filters} must be divisible by groups={groups}")
    ...
    x_splits = tf.split(x, num_or_size_splits=groups, axis=-1)
    ...
    for i, x_group in enumerate(x_splits):
        ...
        layer = _maybe_annotate(layer, annotate_qat)
        y_splits.append(layer(x_group))
    return tf.concat(y_splits, axis=-1, name=f"{name}_concat")
...
def apply_qat_to_annotated_model(annotated_model: tf.keras.Model) -> tf.keras.Model:
    ...
    with tfmot.quantization.keras.quantize_scope(
        {"ScalarPReLU": ScalarPReLU, "SFE": SFE}
    ):
...
def summarize_qat_wrappers(model: tf.keras.Model) -> QATSummary:
    ...
    deconv2d = sum(1 for _, wrapped_cls in wrappers if wrapped_cls == "Conv2DTranspose")
