...
class CombinationSearcher:
    def __init__(
        ...
        qat_cfg = config.get("qat", {}) or {}
        self.qat_enabled = bool(qat_cfg.get("enabled", False))
        self.dynamic_quantize_gru = bool(qat_cfg.get("dynamic_quantize_gru", False))
        self.static_quantize_gru = bool(qat_cfg.get("static_quantize_gru", False))
        if self.dynamic_quantize_gru and self.static_quantize_gru:
            raise ValueError(
                "qat.dynamic_quantize_gru and qat.static_quantize_gru are mutually exclusive."
            )
        self.qat_gru = bool(qat_cfg.get("qat_gru", False))
        scale_constraint_cfg = qat_cfg.get("scale_constraint", {})
        if isinstance(scale_constraint_cfg, Mapping):
            self.scale_constraint_mode = scale_constraint_cfg.get(
                "mode", qat_cfg.get("scale_constraint_mode", "none")
            )
            self.scale_constraint_frac_bits = scale_constraint_cfg.get(
                "frac_bits", qat_cfg.get("scale_constraint_frac_bits", None)
            )
            self.scale_constraint_pow2_rounding = scale_constraint_cfg.get(
                "pow2_rounding", qat_cfg.get("scale_constraint_pow2_rounding", "nearest")
            )
        else:
            self.scale_constraint_mode = (
                scale_constraint_cfg if scale_constraint_cfg is not None else qat_cfg.get("scale_constraint_mode", "none")
            )
            self.scale_constraint_frac_bits = qat_cfg.get("scale_constraint_frac_bits", None)
            self.scale_constraint_pow2_rounding = qat_cfg.get("scale_constraint_pow2_rounding", "nearest")
        if self.qat_enabled:
            self.model.prepare_qat(
                ...
                quantize_linear=bool(qat_cfg.get("quantize_linear", False)),
                quantize_gru=self.qat_gru,
                scale_constraint_mode=self.scale_constraint_mode,
                scale_constraint_frac_bits=self.scale_constraint_frac_bits,
                scale_constraint_pow2_rounding=self.scale_constraint_pow2_rounding,
            )
    ...
    def evaluate_combo(self, combo: dict[str, int]) -> float:
        ...
        if self.objective_mode == "int8":
            model_for_eval = self.model.cpu().convert_qat(
                inplace=False,
                dynamic_quantize_gru=self.dynamic_quantize_gru,
                static_quantize_gru=self.static_quantize_gru,
                dynamic_gru_scale_constraint_mode=self.scale_constraint_mode,
                dynamic_gru_scale_constraint_frac_bits=self.scale_constraint_frac_bits,
                dynamic_gru_scale_constraint_pow2_rounding=self.scale_constraint_pow2_rounding,
            )
