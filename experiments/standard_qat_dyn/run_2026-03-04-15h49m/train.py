...
def run(rank, config, args):
    ...
    if qat_enabled:
        ...
        if (rank == 0) and qat_gru and not dynamic_quantize_gru:
            print(
...
class Trainer:
    def __init__(self, config, model, optimizer, scheduler, loss_func,
        ...
        if (
            self.qat_enabled
            and self.rank == 0
            and self.qat_float_warmup_epochs >= self.qat_disable_observer_epoch
        ):
            print(
                "QAT schedule warning: float_warmup_epochs >= disable_observer_epoch. "
    ...
    def _apply_qat_schedule(self, epoch):
        ...
        if fake_quant_enabled:
            ...
            # Float warmup: collect observer stats with fake quant disabled.
        ...
        if self._qat_fake_quant_enabled_state != fake_quant_enabled:
            if self.rank == 0:
                if fake_quant_enabled:
                    ...
                    print(
                        f"QAT schedule: epoch {epoch} uses float warmup "
                        f"(fake quant disabled, observers active)."
                    )
