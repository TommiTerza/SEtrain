...
def _copy_and_freeze_gru_from_teacher(student_model, teacher_model):
    ...
    for name, student_gru in student_grus.items():
        missing_keys, unexpected_keys = student_gru.load_state_dict(
            teacher_grus[name].state_dict(),
            strict=False,
        )
        bad_missing = [
            key for key in missing_keys if not any(prefix in key for prefix in ignored_prefixes)
        ]
        if bad_missing or unexpected_keys:
            raise RuntimeError(
                "Failed to copy teacher GRU into student. "
                f"module={name}, missing={bad_missing[:10]}, unexpected={unexpected_keys[:10]}"
            )
...
def run(rank, config, args):
    ...
    phase_schedule_cfg = qat_config.get("phase_schedule", {})
    phase_schedule_enabled = False
    qat_start_epoch = 1
    scale_constraint_start_epoch = None
    if isinstance(phase_schedule_cfg, Mapping):
        phase_schedule_enabled = bool(phase_schedule_cfg.get("enabled", False))
        qat_start_epoch = int(phase_schedule_cfg.get("qat_start_epoch", 1) or 1)
        if qat_start_epoch < 1:
            qat_start_epoch = 1
        scale_start_value = phase_schedule_cfg.get("scale_constraint_start_epoch", None)
        if scale_start_value not in {None, "", "none", "null", 0}:
            scale_constraint_start_epoch = int(scale_start_value)
            if scale_constraint_start_epoch < 1:
                scale_constraint_start_epoch = 1
    elif phase_schedule_cfg not in {None, False}:
        phase_schedule_enabled = bool(phase_schedule_cfg)
    ...
    scale_constraint_mode_norm = str(scale_constraint_mode).strip().lower() if scale_constraint_mode is not None else "none"
    delayed_scale_constraint = (
        phase_schedule_enabled
        and (scale_constraint_start_epoch is not None)
        and (scale_constraint_start_epoch > qat_start_epoch)
        and (scale_constraint_mode_norm not in {"", "none", "off", "false", "0"})
    )
    prepare_scale_constraint_mode = "none" if delayed_scale_constraint else scale_constraint_mode
    if phase_schedule_enabled and not qat_enabled:
        raise ValueError("qat.phase_schedule.enabled=True requires qat.enabled=True.")
    if qat_enabled:
        model.prepare_qat(
            ...
            scale_constraint_mode=prepare_scale_constraint_mode,
            scale_constraint_frac_bits=scale_constraint_frac_bits,
            scale_constraint_pow2_rounding=scale_constraint_pow2_rounding,
        )
        if (rank == 0) and phase_schedule_enabled:
            phase_msg = (
                f"QAT phase schedule: qat_start_epoch={qat_start_epoch}, "
                f"scale_constraint_start_epoch={scale_constraint_start_epoch}, "
                f"initial_prepare_scale_mode={prepare_scale_constraint_mode}."
            )
            print(phase_msg)
        if (rank == 0) and qat_gru and not dynamic_quantize_gru and not static_quantize_gru:
            print(
                "QAT config: qat_gru=True but dynamic_quantize_gru=False. "
                "GRU fake-quant will be trained, but exported int8 model will keep float GRUs."
            )
        if (rank == 0) and static_quantize_gru:
            print(
                "QAT config: static_quantize_gru=True. "
                "GRUs will use static Q/DQ conversion (fixed activation qparams, int8 weights)."
            )
        if (rank == 0) and dynamic_quantize_gru:
            print(
...
class Trainer:
    def __init__(self, config, model, optimizer, scheduler, loss_func,
        ...
        scale_constraint_cfg = self.qat_config.get("scale_constraint", {})
        if isinstance(scale_constraint_cfg, Mapping):
            self.qat_requested_scale_constraint_mode = scale_constraint_cfg.get(
                "mode", self.qat_config.get("scale_constraint_mode", "none")
            )
            self.qat_requested_scale_constraint_frac_bits = scale_constraint_cfg.get(
                "frac_bits", self.qat_config.get("scale_constraint_frac_bits", None)
            )
            self.qat_requested_scale_constraint_pow2_rounding = scale_constraint_cfg.get(
                "pow2_rounding", self.qat_config.get("scale_constraint_pow2_rounding", "nearest")
            )
        else:
            self.qat_requested_scale_constraint_mode = (
                scale_constraint_cfg
                if scale_constraint_cfg is not None
                else self.qat_config.get("scale_constraint_mode", "none")
            )
            self.qat_requested_scale_constraint_frac_bits = self.qat_config.get("scale_constraint_frac_bits", None)
            self.qat_requested_scale_constraint_pow2_rounding = self.qat_config.get(
                "scale_constraint_pow2_rounding", "nearest"
            )

        self.qat_phase_schedule_cfg = self.qat_config.get("phase_schedule", {})
        self.qat_phase_schedule_enabled = False
        self.qat_start_epoch = 1
        self.qat_scale_constraint_start_epoch = None
        if isinstance(self.qat_phase_schedule_cfg, Mapping):
            self.qat_phase_schedule_enabled = bool(self.qat_phase_schedule_cfg.get("enabled", False))
            self.qat_start_epoch = int(self.qat_phase_schedule_cfg.get("qat_start_epoch", 1) or 1)
            if self.qat_start_epoch < 1:
                self.qat_start_epoch = 1
            scale_start_value = self.qat_phase_schedule_cfg.get("scale_constraint_start_epoch", None)
            if scale_start_value not in {None, "", "none", "null", 0}:
                self.qat_scale_constraint_start_epoch = int(scale_start_value)
                if self.qat_scale_constraint_start_epoch < 1:
                    self.qat_scale_constraint_start_epoch = 1
        elif self.qat_phase_schedule_cfg not in {None, False}:
            self.qat_phase_schedule_enabled = bool(self.qat_phase_schedule_cfg)

        requested_mode_norm = (
            str(self.qat_requested_scale_constraint_mode).strip().lower()
            if self.qat_requested_scale_constraint_mode is not None
            else "none"
        )
        self.qat_delayed_scale_constraint = (
            self.qat_phase_schedule_enabled
            and (self.qat_scale_constraint_start_epoch is not None)
            and (self.qat_scale_constraint_start_epoch > self.qat_start_epoch)
            and (requested_mode_norm not in {"", "none", "off", "false", "0"})
        )
        self._qat_scale_constraint_applied = not self.qat_delayed_scale_constraint
        ...
        self.qat_float_warmup_epochs = int(self.qat_config.get('float_warmup_epochs', 0) or 0)
        if self.qat_float_warmup_epochs < 0:
            self.qat_float_warmup_epochs = 0
        self._qat_fake_quant_enabled_state = None
        self._qat_observer_enabled_state = None
        fake_quant_start_epoch = (
            self.qat_start_epoch + self.qat_float_warmup_epochs
            if self.qat_phase_schedule_enabled
            else (self.qat_float_warmup_epochs + 1)
        )
        if (
            self.qat_enabled
            and self.rank == 0
            and fake_quant_start_epoch >= self.qat_disable_observer_epoch
        ):
            print(
                "QAT schedule warning: fake-quant start epoch is >= disable_observer_epoch. "
                "Observers may be frozen before fake quant is enabled."
            )
        if (
            self.qat_enabled
            and self.qat_phase_schedule_enabled
            and self.rank == 0
            and self.qat_scale_constraint_start_epoch is not None
            and self.qat_scale_constraint_start_epoch < self.qat_start_epoch
        ):
            print(
                "QAT phase schedule warning: scale_constraint_start_epoch < qat_start_epoch. "
                "Scale constraints will follow the initial QAT configuration."
            )
        ...
        self.qat_best_requires_scale_constraint = bool(self.qat_delayed_scale_constraint)
        self._qat_best_waiting_logged = False
        if self.rank == 0 and self.qat_best_requires_scale_constraint:
            print(
                "QAT best-checkpoint policy: selecting best model only after "
                f"scale constraints are enabled (epoch >= {self.qat_scale_constraint_start_epoch})."
            )
    ...
    def _apply_qat_schedule(self, epoch):
        ...
        if (
            self.qat_delayed_scale_constraint
            and (not self._qat_scale_constraint_applied)
            and (self.qat_scale_constraint_start_epoch is not None)
            and (epoch >= self.qat_scale_constraint_start_epoch)
        ):
            replaced, updated = model.enable_scale_constraints(
                mode=self.qat_requested_scale_constraint_mode,
                frac_bits=self.qat_requested_scale_constraint_frac_bits,
                pow2_rounding=self.qat_requested_scale_constraint_pow2_rounding,
            )
            self._qat_scale_constraint_applied = True
            if self.rank == 0:
                print(
                    "QAT schedule: enabled scale constraints at epoch "
                    f"{epoch} (mode={self.qat_requested_scale_constraint_mode}, "
                    f"replaced_fake_quants={replaced}, updated_fake_quants={updated})."
                )
            self._setup_qat_logging()

        if self.qat_phase_schedule_enabled:
            observer_enabled_by_phase = epoch >= self.qat_start_epoch
            fake_quant_enabled = epoch >= (self.qat_start_epoch + self.qat_float_warmup_epochs)
        else:
            observer_enabled_by_phase = True
            fake_quant_enabled = epoch > self.qat_float_warmup_epochs

        if fake_quant_enabled:
            model.apply(quant.enable_fake_quant)
        else:
            # Float phase / observer warmup: keep fake quant disabled.
            model.apply(quant.disable_fake_quant)

        if self._qat_fake_quant_enabled_state != fake_quant_enabled:
            if self.rank == 0:
                if fake_quant_enabled:
                    print(
                        f"QAT schedule: epoch {epoch} enables fake quant "
                        f"(float warmup epochs={self.qat_float_warmup_epochs})."
                    )
                else:
                    phase_label = (
                        "observer warmup"
                        if observer_enabled_by_phase
                        else "full-float phase"
                    )
                    print(
                        f"QAT schedule: epoch {epoch} uses {phase_label} "
                        f"(fake quant disabled)."
                    )
            self._qat_fake_quant_enabled_state = fake_quant_enabled
        ...
        if self._qat_observer_enabled_state != observer_enabled_by_phase:
            if self.rank == 0:
                if observer_enabled_by_phase:
                    print(f"QAT schedule: epoch {epoch} enables observer collection.")
                else:
                    print(f"QAT schedule: epoch {epoch} keeps full-float phase (QAT observers disabled).")
            self._qat_observer_enabled_state = observer_enabled_by_phase
    ...
    def _save_checkpoint(self, epoch, score):
        ...
        if (
            self.qat_best_requires_scale_constraint
            and self.qat_scale_constraint_start_epoch is not None
            and epoch < self.qat_scale_constraint_start_epoch
        ):
            if (self.rank == 0) and (not self._qat_best_waiting_logged):
                print(
                    "QAT best-checkpoint policy: skipping best-model selection before "
                    f"scale constraint start epoch ({self.qat_scale_constraint_start_epoch})."
                )
                self._qat_best_waiting_logged = True
            return
    ...
    def train(self):
        try:
            ...
            if self.rank == 0:
                if self.state_dict_best is None:
                    raise RuntimeError(
                        "No eligible best checkpoint was selected. "
                        "With QAT phase scheduling enabled, best-model selection is restricted to "
                        "epochs where scale constraints are active. "
                        "Check scale_constraint_start_epoch, epochs, and save_checkpoint_interval."
                    )
