...
## Usage
### Install
1. Install PyTorch (`torch`) (pick a CUDA build if you train on GPU). If you already have `accelerate/torchrl/tensordict` installed, use `torch>=2.6`.
2. Install Python deps:
   ```
   pip install -r requirements.txt
   ```
   Or create a fresh env via:
   ```
   conda env create -f environment.yml
   conda activate setrain
   ```

If `pip` reports dependency conflicts (e.g. `accelerate/torchrl/tensordict` vs. `torch`), your environment is inconsistent: either upgrade `torch` or remove the conflicting packages from that env.
...
5. Run `train.py`:
   ...
   python train.py -C configs/cfg_train.yaml -D 0,1,2,3
	   ...
	   To also quantize decoder `ConvTranspose2d` layers, set `qat.quantize_deconv: True`.
	   To force per-channel weight quantization (including deconvs when using `qat.backend: qnnpack`), set `qat.per_channel_weights: True`.
	   To dynamically quantize all `nn.GRU` layers at int8 conversion time, set `qat.dynamic_quantize_gru: True`.
	   For KD runs where student GRUs must stay identical to the teacher, set `distillation.freeze_gru_from_teacher: True`.
	   For a full comparison of all QAT/KD/blockwise strategies in this repo, see `docs/QAT_STRATEGIES.md`.
...
7. Run `evaluate.py`.
8. To choose the best epoch after training with a custom rule, use `select_best_epoch.py`:
   ```
   python select_best_epoch.py --run-dir experiments/<your_run> --source tensorboard --metric pesq --objective max
   python select_best_epoch.py --run-dir experiments/<your_run> --source tensorboard --metric val_loss --objective min
   python select_best_epoch.py --run-dir experiments/<your_run> --source results --expr "0.7*PESQ + 0.3*ESTOI" --objective max
   ```
   Optional: `--link-name best_model_custom.tar` creates a symlink in `checkpoints/` to the selected epoch checkpoint.

**Dataset**: the default dataloader uses the VoiceBank-DEMAND 16k layout (`<root>/<split>/{noisy,clean}/*.wav`). It will automatically reuse the dataset at `../SEtrain/data/voicebank-demand-16k` if that sibling repo exists; override with `train_dataset.dataset_root` / `validation_dataset.dataset_root` or `$SETRAIN_DATA_ROOT`.
