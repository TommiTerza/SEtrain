...
class IndependentBlockwiseDistiller:
    def __init__(
        ...
        scale_constraint_cfg = self.qat_cfg.get("scale_constraint", {})
        if isinstance(scale_constraint_cfg, Mapping):
            self.scale_constraint_mode = scale_constraint_cfg.get(
                "mode", self.qat_cfg.get("scale_constraint_mode", "none")
            )
            self.scale_constraint_frac_bits = scale_constraint_cfg.get(
                "frac_bits", self.qat_cfg.get("scale_constraint_frac_bits", None)
            )
            self.scale_constraint_pow2_rounding = scale_constraint_cfg.get(
                "pow2_rounding", self.qat_cfg.get("scale_constraint_pow2_rounding", "nearest")
            )
        else:
            self.scale_constraint_mode = (
                scale_constraint_cfg
                if scale_constraint_cfg is not None
                else self.qat_cfg.get("scale_constraint_mode", "none")
            )
            self.scale_constraint_frac_bits = self.qat_cfg.get("scale_constraint_frac_bits", None)
            self.scale_constraint_pow2_rounding = self.qat_cfg.get("scale_constraint_pow2_rounding", "nearest")
        if self.qat_enabled:
            self.student_template.prepare_qat(
                ...
                quantize_linear=bool(self.qat_cfg.get("quantize_linear", False)),
                quantize_gru=bool(self.qat_cfg.get("qat_gru", False)),
        ...
        if self.qat_enabled:
            ...
            self.qat_float_warmup_epochs = int(self.qat_cfg.get("float_warmup_epochs", 0) or 0)
    ...
    def _apply_qat_schedule_to_block(self, block: torch.nn.Module, epoch: int) -> None:
        ...
        if epoch > self.qat_float_warmup_epochs:
            block.apply(quant.enable_fake_quant)
        else:
            block.apply(quant.disable_fake_quant)

        if epoch >= self.qat_disable_observer_epoch:
            block.apply(quant.disable_observer)
        else:
            block.apply(quant.enable_observer)
